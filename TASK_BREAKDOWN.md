...
_No conflicts with others, works on core foundation._
...
_Works on separate feature (chat panel), minimal conflict._
...
_Code Owner, handles remaining features and final integration._
...
## 🔧 PR Workflow & Verification (New)

_Áp dụng cho mọi thay đổi AI/UI: không push thẳng main, luôn qua PR._

### ✅ Nhật (AI) - Done

- [x] **Task 6.x:** Nâng cấp AI chat hiểu câu hỏi không chuẩn (không dấu), trả task theo người, nhóm theo deadline/status, output có cấu trúc.
- [x] **Task 6.y:** Seed dữ liệu demo đa dạng (members/tasks/OKR) để demo ngay khi mở app.

### ✅ Tuấn (Code Owner) - Next steps

- [ ] **Task PR.1:** Rebase `feat/ai-knowledge-manager` lên `origin/main`, resolve conflicts nếu có.
- [ ] **Task PR.2:** Verify trước PR:
  - `node --check script.js`
  - mở UI và test: “Tuấn đang có task gì”, “task quá hạn”, “OKR”, “tổng quan/status”.
- [ ] **Task PR.3:** Push branch lên remote (nếu rebase thì dùng `--force-with-lease`).
- [ ] **Task PR.4:** Tạo PR `main <- feat/ai-knowledge-manager`, review diff, merge sau khi pass QC.
